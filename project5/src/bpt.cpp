...
int db_find(int table_id, element key, char* ret_val,int trx_id) {
    //printf("find key: %d trx_id : %d \n",key,trx_id);
    if(table_id>10||table_id==-1)return -1;
    record_t* record;
    record=(record_t*)malloc(sizeof(record_t));
    trx* trx_;
    if((trx_=find_trx(trx_id))==NULL)return -1;
    //printf("f2\n");k
    if ((record = find_tr(table_id, key,trx_id,SHARED)) == NULL) {
        //printf("find error\n");
        free(record);
        return -1;
    }
...
int db_update(int table_id,element key,char* values,int trx_id){
    //printf("update key: %d trx_id : %d f1\n",key,trx_id);
    if(table_id>10||table_id==-1)return -1;
    record_t* record;
    trx* trx_;
    if((trx_=find_trx(trx_id))==NULL){
         //printf("update error\n");
        return -1;
    }
     if ((record = find_update_trx(table_id, key,values,trx_id,EXCLUSIVE)) == NULL) {
        //printf("update error\n");
        free(record);
        return -1;
    }
   // printf("up2\n");
    ...
    r.key=key;
    r.table_id=table_id;
    strncpy(r.value,record->value,120);
    trx* trx_find=find_trx(trx_id); 
    trx_find->record.push_back(r);
    //printf("push_back %d\n",trx_id);
    free(record);
    return 0;
}
int db_delete(int table_id, element key) {
    if(table_id>10||table_id==-1)return -1;
    if (buf_delete(table_id, key) >= 0)return 0;
    else return -1;
}
...
record_t* find_tr(int table_id, element key,int trx_id,int lock_mode) {
    ...
    if (leaf_num == 0||leaf_num==-1) return NULL;
    //printf("tr1\n");
    ...
    if (i == ((leaf_page_t*)page)->Number_of_Keys) {
        ...
        //rintf("tr3\n");
        put_frame(buf, 0);
        record_lock=lock_acquire(table_id,key,trx_id,lock_mode);
        if(record_lock->deadlock==1){
            //printf("record_lock->deadlock %d\n",record_lock->deadlock);
            ret->key=-1;
            return ret;
        }
        //printf("tr4\n");
        ...
        ret->key = key;
       // printf("fr:%s\n",((leaf_page_t*)page)->record[i].value);
...
record_t* find_update_trx(int table_id, element key,char* values,int trx_id,int lock_mode){
    ...
    if (i == ((leaf_page_t*)page)->Number_of_Keys) {
       ...
       // printf("up2:%s\n",((leaf_page_t*)page)->record[i].value);
        strncpy(ret->value, ((leaf_page_t*)page)->record[i].value, 120);
        strncpy(((leaf_page_t*)page)->record[i].value,values,120);
        //printf("up1:%s\n",values);
       // printf("up2:%s\n",((leaf_page_t*)page)->record[i].value);
        put_frame(buf,1);
       // printf("fu7\n");
...
/* Creates a new genj2l node, which can be adapted
...
pagenum_t insert_into_node_after_splitting(int table_id, pagenum_t parent, int left_index,
    ...
    pagenum_t new_num = make_page(table_id);
    new_buf = get_frame(table_id, new_num);
    page_t* new_page = new_buf->page;
    if (temp_nums == NULL) {
    ...
    ((internal_page_t*)child_page)->parent_page_number = new_num;
    ...
    put_frame(parent_buf, 1);
    put_frame(new_buf, 1);
    return insert_into_parent(table_id, parent, k_prime, new_num);
}
